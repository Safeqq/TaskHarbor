...
Phase 7 runs the durable queue across multiple worker processes and recovers work after a worker crash. Every attempt now has a worker owner, renewable lease, and unique claim token that fences stale processes from progress or final-result writes.
...
6. Each worker registers its name and capacity, sends heartbeats, and claims up to its local concurrency limit. A claim creates an attempt with a lease and a random fencing token.
7. Active workers renew leases while processing. If a lease expires, any worker can transactionally close that attempt and schedule a retry, finish a pending cancellation, or fail the job when its attempt limit is exhausted.
8. Every progress, failure, cancellation, and completion write checks the attempt owner and token. A stale worker can finish physical computation, but PostgreSQL rejects its write and its per-attempt output directory is discarded.
9. PostgreSQL publishes the complete output manifest only after every item succeeds. The dashboard shows lifecycle actions, attempt ownership, schedules, occurrence history, and worker heartbeat status.
